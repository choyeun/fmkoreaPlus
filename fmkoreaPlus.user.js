...
(function () {
    ...
    setInterval(() => {
        ...
        allElements.forEach(function (element) {
            ...
            // 특정 문자열을 포함하는 클래스가 있는지 확인
            if (classNames && Array.from(classNames).some(className => className.includes('member_plate'))) {
                // 배열에 클래스 추가
                aaaa.push(classNames);
            }
        ...
        for (var i = 0; i < aaaa.length; i++) {
            //console.log('확인된 클래스 배열:', foundClasses[i].value);
            //console.log(foundfindParent[i]);
            console.log(document.getElementsByClassName(aaaa[i])[0].innerText);
            console.log(document.getElementsByClassName(aaaa[i])[0].id);

            //console.log(document.getElementsByClassName(foundfindParent[i])[0].innerHTML);
            document.getElementsByClassName(aaaa[i])[0].innerText = "" // 대댓글 감지
            document.getElementsByClassName(aaaa[i])[i].innerHTML = "" // 대댓글 감지 2
        }
